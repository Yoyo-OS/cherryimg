    ...
    , m_currentFontSize(0)
{
    ...
    // Info field
    m_exif_base = new QWidget(this);
    m_exif_base->setFixedWidth(280);

    m_exif_details = new QWidget(this);
...
//    m_labelBasic = new QLabel(tr("Basic info"));
//    m_labelBasic->setFixedHeight(100);
//    m_labelDetails = new QLabel(tr("Details"));
//    m_mainLayout->addWidget(m_labelBasic );
    m_mainLayout->addWidget(m_exif_base );
//    m_mainLayout->addWidget(m_labelDetails);
...
void LibImageInfoWidget::updateInfo()
{
    ...
    auto mds = LibUnionImage_NameSpace::getAllMetaData(m_path);
...
//        QPalette pa1 = DApplicationHelper::instance()->palette(field);
...
//        title->setPalette(pa2);
        ...
        qDebug()<<fm.height() * list.size();
        title->setFixedHeight(fm.height() * list.size());
        title->setFixedHeight(50);

        m_exifLayout_base->addRow(title, field);
        m_exif_base->setFixedHeight(m_exifLayout_base->count() * 20);
        this->setFixedHeight(m_exifLayout_base->count() * 20);
    }
}
...
//        title->setPalette(pa2);
        ...
        //        m_exif_details->setFixedHeight(index*30);
        m_exifLayout_base->addRow(title, field);
...
QList<QLabel *> LibImageInfoWidget::addExpandWidget(const QStringList &titleList)
{
    QVBoxLayout *layout = qobject_cast<QVBoxLayout *>(this->layout());
    QList<QLabel *> group;
    ...
    for (const QString &title : titleList) {
        ...
        QLabel *expand = new QLabel;  // DArrowLineExpand;
        expand->setText(title);
        expand->setFixedHeight(50);
...
void LibImageInfoWidget::initExpand(QVBoxLayout *layout, QLabel *expand)
{
...
int LibImageInfoWidget::contentHeight() const
{
    ...
    foreach (const QLabel *expand, m_expandGroup) {
