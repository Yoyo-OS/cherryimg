...
//    QRegExp rx("[^\\\\//:*?\"<>|]*");
    ...
    connect(m_lineedt, &QLineEdit::textChanged, this, [ = ](const QString & arg) {
        setCurrentTip();
    ...
    connect(m_lineedt, &QLineEdit::textEdited, this, [ = ](const QString & arg) {
        if (arg.isEmpty()) {
            return;
        }
        if (arg.at(0) == " ") {
            ...
            m_lineedt->setText(str);
            m_lineedt->setCursorPosition(0);
        }
        int len = arg.toLocal8Bit().length();
        ...
        if (len > 256 - Dirlen) {
            ...
            m_lineedt->setText(Interceptstr);
        };
    });
